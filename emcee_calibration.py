...
if __name__ == "__main__":
    ...
    ## This is currently commented out because it doesn't converge
    ...
    with Pool() as pool:
        sampler = emcee.EnsembleSampler(n_walkers, n_dim, post, pool=pool)
        start_time = time.time()
        sampler.run_mcmc(theta0, 100, progress=True)
        end_time = time.time()
    print(f"Total time for calibration: {end_time - start_time}")
    ...
    with PdfPages(fig_name) as pdf:
        ...
        fig = corner.corner(
            flat_samples,
            labels=[r"$C_{\mu}$", r"$C_{1e}$", r"$C_{2e}$"],
            truths=theta_true,
        )
